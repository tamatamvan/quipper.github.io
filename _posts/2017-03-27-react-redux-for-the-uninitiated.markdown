...
I hear the words _the New Hotness_ thrown a lot. Curiously, former Hotness Coffeescript has now been relegated to the most hated list.
...
So far, so good. All of the action takes place in that render call. So this will be like the program's entry point. It takes 2 arguments, the markup and the root node. To take things further, we would need to extract the markup<sup>[6](#footnote6)</sup> from there and build a proper React class.
...
One of the big thing about react is something called the State. It's like a data representation of the app. I like to think of it as a switchboard that shows and controls all of the app's behavior. Best way to illustrate it is probably by code.

Say you want to show a modal. In event-driven thinking, this could be accomplished by adding a button event handler to insert the modal into your DOM. But in react, you only need to turn the modal "on" from the State.
...
Okay, so the State seem to be just a big list of variables that represents various ehem, status of the app.  Here we're using the State to represent the visibility of the modal. Any changes in that state will instantly be reflected in our render method. Suffice to say, one of the reason for react's success is its Virtual DOM implementation. it solves the problem of having to re-render the entire DOM which, overtime, will cost time and resource.[attribution]

### Redux

Though not in the same developer survey, one of the things that seem to excite a lot of developers (I know) today is Redux. It's basically a souped up State when React's State is not enough<sup>[7](#footnote7)</sup>. The website tells it better, Redux is a predictable state container for Javascript apps (which has got to be one of the best written product bylines I've seen).
To be honest, I'm not sure when React State is not enough or when Redux should come in<sup>[8](#footnote8)</sup>. But just for the purposes of illustration, let's convert our trivial app to use Redux.
First, we'll need to load one more external library.
...
then instead of calling the global store, we can access it via the local context
...
Funny, our code keeps getting longer and longer and no single feature has been added.
...
<small>
<a name="footnote1">1</a>: [http://stackoverflow.com/insights/survey/2017/](http://stackoverflow.com/insights/survey/2017/)<br>
...
<a name="footnote7">7</a> Okay, maybe this is a gross oversimplification. Redux takes after [Flux](http://redux.js.org/docs/introduction/PriorArt.html).<br>It's also said that it's one of the best implementation of flux that's [not really flux](https://twitter.com/andrestaltz/status/616271392930201604)<br>
<a name="footnote8">8</a> There's an adequately comprehensive series of video tutorials from Redux's author himself [here](https://egghead.io/courses/getting-started-with-redux)
</small>
